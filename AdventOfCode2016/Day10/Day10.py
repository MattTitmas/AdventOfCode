...
def part1():
    ...
    for command in file:
        if command[0] == "value":
            value = int(command[1])
            botNo = int(command[5])
            botsHolding[botNo] = [value] if botNo not in botsHolding else sorted(botsHolding[botNo]+[value])
        else:
            botNo = int(command[1])
    ...
    while True:
        ...
        for key, val in copy.items():
            if len(val) == 2:
                ...
                if not low[1]:
                    botsHolding[low[0]] = [val[0]] if low[0] not in botsHolding else sorted(botsHolding[low[0]] + [val[0]])
                else:
                    outputs[low[0]] = [val[0]] if low[0] not in outputs else sorted(outputs[low[0]] + [val[0]])
                if not high[1]:
                    botsHolding[high[0]] = [val[1]] if high[0] not in botsHolding else sorted(botsHolding[high[0]] + [val[1]])
                else:
                    outputs[high[0]] = [val[1]] if high[0] not in outputs else sorted(outputs[high[0]] + [val[1]])
                botsHolding[key] = []
