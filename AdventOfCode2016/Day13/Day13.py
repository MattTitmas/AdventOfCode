    ...
    while (wantedxPos, wantedyPos) not in foundPositions:
        ...
        for i in range(max(0, currentX - 1), currentX + 2):
            for j in range(max(0, currentY - 1), currentY + 2):
                if (
                    abs(currentX - i) != abs(currentY - j)
                    and not sum(
                        int(x)
                        for x in bin(i * i + 3 * i + 2 * i * j + j + j * j + file)[2:]
                    )
                    % 2
                ):
        ...
        for neighbour in neighbours:
            if neighbour not in foundPositions:
                currentDistances[neighbour] = (
                    lowestDistance + 1
                    if neighbour not in currentDistances
                    else min(currentDistances[neighbour], lowestDistance + 1)
                )
