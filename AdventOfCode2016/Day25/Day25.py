...
def part1():
    ...
    while True:
        ...
        registers = {'a': currentTest, 'b': 0, 'c': 0, 'd': 0}
        i = 0
        toPrint = ""
        while i < len(file):
            command = file[i]
            if command[0] == "cpy":
                ...
                toPrint += str(int(command[1]) if command[1].lstrip("-").isnumeric() else registers[command[1]])
            elif command[0] == "tgl":
            ...
            if len(toPrint) > 200:
                break
        for i in range(200):
            if toPrint[i] != str(int(i) % 2):
                break
        else:
            print(currentTest, toPrint)
        currentTest += 1
