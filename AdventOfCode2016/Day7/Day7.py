...
def containsABBA(string : str) -> bool:
    for count in range(0, len(string)-3):
        ...
        next = string[count+1]
        if current != next and string[count+2] == next and string[count+3] == current:
            return True
    return False
                    ...
                    for string2 in strings:
                        if string2[0] == "_":
                            for count2 in range(1, len(string2) - 2):
                                if string2[count2] == inside and string2[count2+1] == outside and string2[count2+2] == inside:
                                    return True
    return False

def part1():
    values = [i.replace("[","]_").split("]") for i in open("input.txt").read().split("\n")]
    total = 0
